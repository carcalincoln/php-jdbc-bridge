...
public class Utils {
	...
	private static int makeCUID_i = 1;
	...
	/**
	 * Split a string into words. Needed to parse the commands from the PHP backend
	...
	/**
	 * Split a file into words. Needed to read the main config file.
	 * 
	...
	/**
	 * Create an UID. Needed to keep track of the JDBC ResultSets with the PHP
	 * backend.
	 * 
	...
	public static synchronized String makeCUID() {

		if ((makeCUID_i + 1) == Integer.MAX_VALUE)
			makeCUID_i = 0;

		return new String("PJBS_Cid_" + Integer.toString(makeCUID_i++));
	}
